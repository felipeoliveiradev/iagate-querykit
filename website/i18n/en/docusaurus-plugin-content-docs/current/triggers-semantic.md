...
Application‑level triggers with `when`, `action`, `except`, `table`, and `body`.
...
action as array and multiple tables:
```ts
...
body: string / function / array / parallel:
```ts
