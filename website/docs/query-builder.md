...
O `QueryBuilder<T>(table)` é a API fluente para construir SQL tipado e executar via um executor configurado.
...
Agrupamento / HAVING / Ordenação / Paginação:
```ts
